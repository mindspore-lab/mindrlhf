...
class TestQwen2_5DPO:
    ...
    def test_qwen2_5_predict(self):
        sh_path = os.path.split(os.path.realpath(__file__))[0]
        scripts_path = f"{root_path}/run_dpo.py"

        scripts_cmd = f"{scripts_path} --config={root_path}/model_configs/qwen_config/predict_qwen2_5_7b.yaml " + \
                      f"--vocab_file={root_path}/checkpoint_download/qwen2_5/vocab.json " + \
                      f"--merges_file={root_path}/checkpoint_download/qwen2_5/merges.txt " + \
